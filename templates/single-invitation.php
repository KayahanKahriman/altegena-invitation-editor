 ...
 *
 * Rendered for single `sie_invitation` posts via SIE_Share_Handler::load_single_template().
 * The card itself is drawn client-side by public-invitation.js from the localized
 * `sie_share.config`; the stored PNG is used as a <noscript> fallback and OG image.
 */
...
$wa_href   = 'https://wa.me/?text=' . rawurlencode('Davetiyemize göz atın: ' . $page_url);
?>
<!DOCTYPE html>
...
<body <?php body_class('sie-public-page'); ?>>

    <div class="sie-public-stage">
        <div id="sie-public-app">
            <?php if (!empty($image_url)) : ?>
                <noscript>
                    <img class="sie-public-fallback-img" src="<?php echo esc_url($image_url); ?>" alt="Davetiye" />
                </noscript>
            <?php endif; ?>
        </div>
    </div>

    <a id="sie-public-share" class="sie-public-share" href="<?php echo esc_url($wa_href); ?>" target="_blank" rel="noopener">
        <span class="sie-public-share-icon" aria-hidden="true">
        ...
        <span class="sie-public-share-label">WhatsApp'ta Paylaş</span>
    </a>
