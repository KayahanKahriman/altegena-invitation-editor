 ...
 * `sie_invitation` post reachable at /davetiye/{token}, generates the
...
class SIE_Share_Handler
{

    private static $instance = null;

    const POST_TYPE = 'sie_invitation';
    ...
    private function __construct()
    {
        ...
        add_action('wp_ajax_sie_save_share', array($this, 'ajax_save_share'));
        add_action('wp_ajax_nopriv_sie_save_share', array($this, 'ajax_save_share'));
    ...
    {
        if (get_option('sie_rewrite_version') !== SIE_VERSION) {
            flush_rewrite_rules(false);
            update_option('sie_rewrite_version', SIE_VERSION);
        }
    }
    ...
    {
        // 1. CSRF
        if (!check_ajax_referer('sie_share', 'nonce', false)) {
        ...
        $max_config = defined('SIE_SHARE_MAX_CONFIG_BYTES') ? SIE_SHARE_MAX_CONFIG_BYTES : 262144;
    ...
    private function sideload_data_url_image($data_url)
    {
        ...
        $max_bytes = defined('SIE_SHARE_MAX_IMAGE_BYTES') ? SIE_SHARE_MAX_IMAGE_BYTES : 5242880;
        ...
        $filename = 'sie-davetiye-' . wp_generate_password(8, false, false) . '.' . $ext;
    ...
    private function create_share_post($config, $image, $product_id)
    {
        ...
        update_post_meta($post_id, '_sie_share_config', wp_slash(wp_json_encode($config)));
        update_post_meta($post_id, '_sie_share_image_id', $image['id']);
        update_post_meta($post_id, '_sie_share_image_url', esc_url_raw($image['url']));
        update_post_meta($post_id, '_sie_share_image_dims', array('w' => $image['width'], 'h' => $image['height']));
        update_post_meta($post_id, '_sie_share_product_id', $product_id);
        update_post_meta($post_id, '_sie_share_ip', md5($this->get_client_ip() . wp_salt()));
    ...
    public function load_single_template($template)
    {
        if (is_singular(self::POST_TYPE)) {
            $custom = SIE_PLUGIN_DIR . 'templates/single-invitation.php';
    ...
    public function enqueue_public_assets()
    {
        ...
        $raw_config = get_post_meta($post_id, '_sie_share_config', true);
        $image_url = get_post_meta($post_id, '_sie_share_image_url', true);
        if (empty($raw_config)) {
            return;
        }

        $fonts_ver = filemtime(SIE_PLUGIN_DIR . 'assets/css/fonts.css');
        $css_ver = filemtime(SIE_PLUGIN_DIR . 'assets/css/public-invitation.css');
        $js_ver = filemtime(SIE_PLUGIN_DIR . 'assets/js/public-invitation.js');

        wp_enqueue_style('sie-fonts-css', SIE_PLUGIN_URL . 'assets/css/fonts.css', array(), $fonts_ver);
        wp_enqueue_style('sie-public-css', SIE_PLUGIN_URL . 'assets/css/public-invitation.css', array('sie-fonts-css'), $css_ver);
        wp_enqueue_script('sie-public-js', SIE_PLUGIN_URL . 'assets/js/public-invitation.js', array('jquery'), $js_ver, true);

        wp_localize_script('sie-public-js', 'sie_share', array(
    ...
    {
        return 'sie_share_rl_' . md5($this->get_client_ip() . wp_salt());
    }
